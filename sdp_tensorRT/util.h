...
namespace util
{
    ...
    {
    public:
        RGBMatReader(const cv::Mat& img, const nvinfer1::Dims& dims) :mImg{img}, mDims{ dims } {};
        RGBMatReader(const std::string& filename, const nvinfer1::Dims& dims);
        std::unique_ptr<float_t> process() const;
    protected:
        nvinfer1::Dims mDims;
        cv::Mat mImg;
    };
